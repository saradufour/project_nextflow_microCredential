process THERMORAWFILEPARSER {
    //publishDir "${params.outdir}/parsed/", mode: 'copy', overwrite: true  //, pattern: "*.bam"
    ...
    input:
    tuple val(meta), val(sample), path(raw)

    output:
    tuple val(sample), path("*.{mzML,mzML.gz,mgf,mgf.gz,parquet,parquet.gz}"), emit: spectra
